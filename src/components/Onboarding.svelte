...
<Card class="mt-2 text-center" size="lg" padding="xl">
	...
	<p class="mb-5 text-balance text-base text-gray-500 dark:text-gray-400 sm:text-lg">
		{$_('onboardingBitcoin.description')}
	</p>
	<div
		class="items-center justify-center space-y-4 sm:flex sm:space-x-4 sm:space-y-0 rtl:space-x-reverse"
	>
		<Button on:click={() => (defaultModal = true)}>{$_('onboardingBitcoin.start')}</Button>
		<Modal title={$_('onboardingBitcoin.title')} bind:open={defaultModal} autoclose outsideclose>
			<Tabs
				style="full"
			...
			>
				<TabItem class="w-full divide-y" open>
					<span slot="title">{$_('onboardingBitcoin.wallet')}</span>
					<p class="text-balance text-sm text-gray-500 dark:text-gray-400">
						<b>{$_('onboardingBitcoin.wallet')}:</b>
						{$_('onboardingBitcoin.walletDescription')} [
						<a href="https://mutinywallet.com" class="underline decoration-dotted">Mutiny</a>
						|
						<a href="https://phoenix.acinq.co/" class="underline decoration-dotted">Phoenix</a> ]
					</p>
					<div class="my-4 pt-4">
						<Button class="mr-2" color="alternative"
							>{$_('onboardingBitcoin.guides')}<ChevronDownSolid
						...
						<Dropdown>
							<DropdownItem
								...
								>{$_('onboardingBitcoin.lnaddress')}</DropdownItem
							>
							<DropdownItem
								...
								>{$_('onboardingBitcoin.lndhub')}</DropdownItem
							>
						</Dropdown>
						<Button href="https://bitcointxoko.com">{$_('onboardingBitcoin.wallet')}</Button>
					</div>
				</TabItem>
				<TabItem class="w-full divide-y">
					<span slot="title">{$_('onboardingBitcoin.boltcard')}</span>
					<p class="text-balance text-sm text-gray-500 dark:text-gray-400">
						<b>{$_('onboardingBitcoin.boltcard')}:</b>
						{$_('onboardingBitcoin.boltcardDescription')}
					</p>
					<div class="my-4 pt-4">
						<Button
							class="mr-2"
						...
						>
						<Button href="/shop">{$_('onboardingBitcoin.shop')}</Button>
					</div>
				</TabItem>
				<TabItem class="w-full divide-y">
					<span slot="title">{$_('onboardingBitcoin.shop')}</span>
					<p class="text-balance text-sm text-gray-500 dark:text-gray-400">
						<b>{$_('onboardingBitcoin.shop')}:</b>
						{$_('onboardingBitcoin.shopDescription')}
					</p>
					<div class="my-4 pt-4">
						<Button
							class="mr-2"
						...
						<Button href="https://btcpay.bitcointxoko.com/register"
							>{$_('onboardingBitcoin.register')}</Button
						>
					</div>
				</TabItem>
			</Tabs>
