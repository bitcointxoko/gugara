<script lang="ts">
    ...
    let showZapModal: boolean = false;
    const openModal = () => {
        showZapModal = true;
    };
    const closeModal = () => {
...
<button on:click={openModal} class="flex items-center">
    {#if !alreadyZapped}
        <Zap
            strokeWidth="2"
            class="mr-1 w-4 h-4 lg:w-4 lg:h-5 stroke-yellow-500 hover:fill-yellow-500"
        />
    {:else if alreadyZapped}
        <Zap
            strokeWidth="2"
            class="mr-1 w-4 h-4 lg:w-4 lg:h-5 fill-yellow-500 stroke-yellow-500 hover:fill-yellow-500"
        />
    {/if}
    {#if totalZaps > 0}
        {totalZaps}
...
<Modal isOpen={showZapModal} onClose={closeModal} title={$_("zap.title")}>
        ...
        {#if $currentUser}
            <form on:submit|preventDefault={zap} class="grid gap-2">
                ...
                <Button type="submit" class="mt-1 w-full">
                    <Zap
                        strokeWidth="2"
                        class="mr-2 w-4 h-4 lg:w-5 lg:h-5 fill-yellow-500 stroke-yellow-500"
                    />
                    {$_("zap.action")}
                </Button>
            </form>
